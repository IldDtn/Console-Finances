...
let netIncome = [];
for (let i = 0; i < flatdata.length; i++) {
    if (typeof flatdata [i]=== 'number') {
        netIncome.push(flatdata[i]);
    }
}
...
const sum = netIncome.reduce((accumulator, value) => {
...
for (i = 0; i< netIncome.length; i++) {
    diff.push(getDifference (netIncome[i], netIncome[i + 1]));
}
